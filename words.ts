...
/**  This method replaces word values with corresponding map key values
     @param words - array of words that are being mutated
     @param reps - Map of the keys words to replacedment word values
     @returns - none because the function just replaces
 */
...
export const substitute = (words: string[], reps: Map<string, string>): void => {
  ...
  // Inv: words = subsitute(words_0[0 ... j - 1], reps).push(words_0[j ... words.length - 1])
  while(j < words.length){
    if(reps.has(words[j])){
      const word : string | undefined = reps.get(words[j]);
 ...
 */
 export function splitWords(str: string): string[] {
  ...
  while (j !== str.length) {  // TODO (part 6a): fix this | was j !== 9

    if (j === 0) {
      splits.push(j + 1);
    } else if (str[j - 1] === " " || str[j] === " ") {
      splits.push(j + 1);
    } else if (isPunct(str[j - 1]) || isPunct(str[j])) {
      splits.push(j + 1);
    } else {
 ...
 */
 export function joinWords(words: ReadonlyArray<string>): string {
  ...
  while (j !== words.length) {
    if(j > 0 &&  !isPunct(words[j][words[j].length - 1]) ){
      result.push(" " + words[j]);
    }else{
      result.push(words[j]);
    }
